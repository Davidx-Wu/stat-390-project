...
def is_neg_subpoint_line(text: str) -> bool:
    ...
    if t.endswith("."):
        return True
    if re.match(r"^(perm|link|impact|internal|uq|uniqueness|answers?|at:|2nc|1nr|2nr|overview|kick|solvency|adv|advantage|inherency)\b", low):
        return True
...
def is_neg_position_header(text: str, style: str) -> bool:
    ...
    if not is_short_heading_candidate(t):
        return False
    if is_generic_scaffolding_line(t):
        return False
    if is_neg_subpoint_line(t):
        return False

    # Strong top-level position titles only.
    if re.search(r"\b(counterplan|cp|disadvantage|da|kritik|topicality|pic)\b", low):
        return True
    if re.fullmatch(r"[A-Za-z0-9 /&+\-]{1,30}\s+k:?", t, flags=re.I):
        return True
    ...
    # Short shell names like "NLRB CP", "RATES DA", "CAP K"
    if len(t) <= 35 and re.search(r"\b(cp|da|k|pic|topicality|theory|framework|fw)\b", low):
        return True
...
def parse_neg_positions(paragraphs: List[Dict[str, str]]) -> List[PositionRecord]:
    ...
    for row in paragraphs:
        ...
        if SPEECH_LABEL_RE.match(text):
            continue
        if is_generic_scaffolding_line(text):
            continue

        if is_neg_position_header(text, style):
            if current is not None:
                current["end_paragraph"] = idx - 1
                positions.append(_finalize_position(current))
            order += 1
