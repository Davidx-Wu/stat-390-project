...
Example:
    python debate_doc_parser.py \
        --doc "Michigan-BaPa-Neg-Georgetown-Round-5.docx" \
        --tournament-csv "Georgetown_Tabroom-prelims_table.csv" \
        --outdir "./outputs" \
        --strict
...
CARD_START_RE = re.compile(
    r"""^
    [A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ.\-,'&/ ]{1,100}
    \s*[’']\d{2}\b
    """,
    re.VERBOSE,
)
...
def safe_excerpt(text: str, n: int = 240) -> str:
...
def derive_team_code_variants(raw: str) -> List[str]:
    ...
    if letters:
        ...
        if len(letters) >= 2:
            initials2 = letters[0].upper() + letters[-1].upper()
            variants.add(initials2)
            variants.add(initials2[::-1])
...
def paragraphs_from_docx(docx_path: Path) -> List[Dict[str, str]]:
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        for run in p.runs:
            ...
            # Some debate docs use underline as the "read" marker.
...
def load_and_match_tournament_row(
    ...
    team_variants = derive_team_code_variants(filename_team_code)
    target_team_variants = set()
    for variant in team_variants:
        target_team_variants.add(normalize_team_code_for_matching(f"Michigan {variant}"))
        target_team_variants.add(normalize_team_code_for_matching(f"University of Michigan {variant}"))
        target_team_variants.add(normalize_team_code_for_matching(variant))
...
def is_short_heading_candidate(text: str) -> bool:
    return len(clean_text(text)) <= 120
...
def is_neg_case_boundary(text: str) -> bool:
    ...
    if re.match(r"^adv\s*---", low):
        return True
    if re.match(r"^advantage(\s+\w+)?\s*[:\-]", low):
        return True
    return False
...
def build_cards_for_position(position: Dict, paragraphs: List[Dict[str, str]]) -> List[Dict]:
    ...
    card_starts = [i for i, p in enumerate(pos_paras) if is_card_start(p["text"])]
    cards: List[Dict] = []

    if not card_starts:
        return cards

    for card_order, rel_start in enumerate(card_starts, start=1):
        rel_end = (card_starts[card_order] - 1) if card_order < len(card_starts) else (len(pos_paras) - 1)
        chunk = pos_paras[rel_start : rel_end + 1]
        cite_line = chunk[0]["text"]
...
def make_summary(
    ...
    if round_info and round_info.team_code_csv:
        ...
        preferred_code = next(
            (v for v in derive_team_code_variants(file_meta.team_code_filename) if len(v) == 2 and v.isupper()),
...
def main() -> int:
    ...
    summary = make_summary(file_meta=file_meta, round_info=round_info, argument_rows=argument_rows, card_rows=card_rows)
    ...
    print(f"Argument audit written: {outdir / 'argument_audit.csv'}")
    print(f"Card audit written: {outdir / 'card_audit.csv'}")
