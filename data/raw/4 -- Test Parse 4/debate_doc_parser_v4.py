...
def is_neg_case_boundary(text: str) -> bool:
    """
    Neg files often continue into case answers / AT advantages / later rebuttal blocks.
    Once we hit the first explicit advantage marker, stop counting new offs.
    """
    t = clean_text(text)
    low = t.lower()
    if not t:
        return False
    if re.match(r"^(adv|advantage)\s*---", low):
        return True
    if re.match(r"^advantage\b", low):
        return True
    if re.match(r"^at\s*[:-]", low):
        return True
    return False
...
def parse_neg_positions(paragraphs: List[Dict[str, str]]) -> List[PositionRecord]:
    ...
    for row in paragraphs:
        ...
        # Hard stop: once the doc transitions into AT advantages / case answers,
        # stop parsing neg offs entirely.
