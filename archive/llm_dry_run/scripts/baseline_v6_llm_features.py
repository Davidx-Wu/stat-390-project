...
LLM_FEATURES_PATH = REPO / "4 -- results" / "llm_features" / "llm_argument_features_v6.csv"
OUT_DIR = REPO / "4 -- results" / "baseline_runs"
METRICS_OUT = OUT_DIR / "baseline_v6_llm_features_metrics.csv"
COEFFICIENTS_OUT = OUT_DIR / "baseline_v6_llm_features_coefficients.csv"
...
def main():
    if not LLM_FEATURES_PATH.exists():
        raise FileNotFoundError(
            f"LLM feature file does not exist: {LLM_FEATURES_PATH}. "
            "Run build_llm_argument_features_v6.py after a usable text column is available."
        )

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    df = pd.read_csv(DATA_PATH).reset_index(names="row_id")
    llm_df = pd.read_csv(LLM_FEATURES_PATH)
    missing_features = [feature for feature in LLM_FEATURES if feature not in llm_df.columns]
    if missing_features:
        raise ValueError(f"LLM feature file is missing columns: {missing_features}")

    merged = df.merge(llm_df[["row_id"] + LLM_FEATURES], on="row_id", how="inner")
    train_df = merged[merged[SPLIT_COL] == TRAIN_SPLIT].copy()
    validation_df = merged[merged[SPLIT_COL] == VALIDATION_SPLIT].copy()

    if train_df.empty:
        raise ValueError("No train rows with LLM features found.")
    if validation_df.empty:
        raise ValueError("No validation rows with LLM features found.")
    ...
    v6_delta = None if v3_validation_accuracy is None else validation_accuracy - v3_validation_accuracy
    ...
    metrics = pd.DataFrame(
        [
            ...
            {"metric": "v6_minus_v3_validation_accuracy", "value": v6_delta},
    ...
    print("Baseline v6: LLM argument-quality features")
    ...
    if v3_validation_accuracy is not None:
        ...
        print(f"V6 minus v3 validation accuracy: {v6_delta:.4f}")
